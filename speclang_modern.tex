...
\begin{example}
  The following definitions \listinginput{1}{signdef.c} illustrates the
...
\begin{itemize}
\item \valid\indexttbs{valid} applies to a set of terms 
(see Section~\ref{sec:locations}) of some pointer type (different to \lstinline|void*|). 
...
applies to a set of terms of some pointer type (different to \lstinline|void*|)
...
some pointer (having a type different to \lstinline|void*|)
...
In fact, like the \assigns clause does not specify which memory location is
assigned,
...
Loop annotations are complemented by similar clauses allowing to specify which 
memory location 
...
\begin{example}
  \notimplemented{The following example illustrates the use of a data invariant on a local static variable.}
...
(having a type different to \lstinline|void*|) to l-values as
