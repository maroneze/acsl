...
\subsubsection{Allocation clauses for function 
...
\begin{listing-nonumber}
\allocable{L}(p) <==> (\allocation{L}(p)==\unallocated && p==\base_addr{L}(p)).
\end{listing-nonumber}
...
For any pointer \lstinline|p| and label \lstinline|L|
\begin{listing-nonumber}
\freeable{L}(p) <==> (\allocation{L}(p)==\dynamic && p==\base_addr{L}(p)).
\end{listing-nonumber}
...
\begin{listing-nonumber}
\fresh{L$_0$,L$_1$}(p,n) <==> (\allocable{L$_0$}(p) && 
                         \freeable{L$_1$}(p) && 
